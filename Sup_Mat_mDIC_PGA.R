...
niter <- 2000  #Number of iterations after burn-in
nthin <- 1     #Thinning factor

data_jags <- list(N=nobse, Y=yresp, X=xresp)
param_jags <- c("r","lambda")
...
mu_mcmc <- unlist(mcmc2_s[,"r"])  # alpha and beta sampled
sig_mcmc <- unlist(mcmc2_s[,"lambda"])    # for each iteration in MCMC
